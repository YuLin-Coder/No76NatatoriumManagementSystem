...
public class SpcategoryBuilder {
	...
	public String BuildNavMenu(){
		...
		List<Spcategory> parentlist=spcategorySrv.getEntity("where isleaf=0 ");
		...
		for(Iterator<Spcategory> iterator = parentlist.iterator();iterator.hasNext();){
			...
			List<Spcategory> listchild=spcategorySrv.getEntity("where parentid="+spc.getId());
        ...
        List<Spcategory> rootleaf=spcategorySrv.getEntity("where isleaf=1 and parentid=0 ");
        sb.append("<ul>");
		for(Iterator<Spcategory> iterator = rootleaf.iterator();iterator.hasNext();){
			...
			sb.append(MessageFormat.format("<li><a href=''{0}/e/shangpinlist.jsp?spcid={1}''>{2}</a></li>",request.getContextPath(), spc.getId(), spc.getMingcheng()));
